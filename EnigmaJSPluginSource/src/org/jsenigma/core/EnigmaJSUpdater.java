...
public class EnigmaJSUpdater
	{
	public static final String svn = "http://svn.github.com/amorri40/EnigmaJS.git"; //$NON-NLS-1$
	...
	public static int checkForUpdates(final EnigmaJSFrame ef)
		{
		EnigmaJSUpdater svn = new EnigmaJSUpdater();
		try
			{
				...
				svn.checkout(repo,new ISVNEventHandler()
					{
						...
						public void checkCancelled() throws SVNCancelException
							{
							if (JSEnigmaRunner.SHUTDOWN) throw new SVNCancelException();
							}
					});
	...
	private EnigmaJSUpdater()
		{
		if (path == null)
			{
			path = JSEnigmaRunner.WORKDIR;
	...
	private static File getThisFile()
		...
		try
			{
			return new File(
					JSEnigmaRunner.class.getProtectionDomain().getCodeSource().getLocation().toURI());
			}
		catch (Exception e)
			{
			e.printStackTrace();
