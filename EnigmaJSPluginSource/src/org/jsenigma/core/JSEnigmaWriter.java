...
public class JSEnigmaWriter {
	...
	public static void initJavascript() throws FileNotFoundException, ScriptException {
	...
	public static void writeResources() throws Exception {
		...
		loadingfile.close();
		if (numberOfErrors>0)
		System.out.println("Failed to convert with "+numberOfErrors+" errors");
		else
...
*/ /*
			...
			if (instanceCount != 0)
				...
				for (int i = 0; i < instanceCount; i++)
					...
					String instancename=((ResourceReference<?>)ii.properties.get(PInstance.OBJECT)).get().getName();
					loadingfile.write("\n new enigma.objects."+instancename+"("+ii.properties.get(PInstance.ID)+","+toId(ii.properties.get(PInstance.OBJECT),-1)+","+ii.properties.get(PInstance.X)+","+ii.properties.get(PInstance.Y)+");");
...
/*
	...
	private static void populateObjects(BufferedWriter loadingfile) throws Exception {
		...
		for (int s = 0; s < size; s++)
			...
			loadingfile.write("\nenigma.objects."+io.getName()+" = function(id, oid, x, y) {");
			loadingfile.write("this.prototype = new enigma.objects.object_locals(id, oid, x, y);");
			loadingfile.write(" this.id=id;	this.x=x; this.y=y;");
			loadingfile.write("this.sprite_index = "+toId(io.get(PGmObject.SPRITE),-1)+"; this.visible = "+io.get(PGmObject.VISIBLE)+"; this.solid = "+io.get(PGmObject.SOLID)+"; this.persistent="+io.get(PGmObject.PERSISTENT)+"; this.parent="+toId(io.get(PGmObject.PARENT),-100)+";this.mask="+toId(io.get(PGmObject.MASK),-1)+";");
			...
			for (int me = 0; me < numevents; me++)
				...
				for (int e = 0; e < eventCount; e++)
					...
					loadingfile.write("\nthis.event_"+ie.mainId+"_"+id+"=function(){");
			...
			loadingfile.write("this.myevent_draw=function() {if (this.image_single!=-1) { this.image_speed = 0; this.image_index = this.image_single; } \n");
			loadingfile.write("if (this.visible && this.sprite_index != -1) enigma.global.draw_sprite_ext(this.sprite_index,this.image_index,this.x,this.y,this.image_xscale,this.image_yscale,this.image_angle,this.image_blend,this.image_alpha);};\n");
			
			loadingfile.write("enigma.classes.depth(this, "+io.get(PGmObject.DEPTH)+");"); //do this last after draw event
			loadingfile.write("};\n");//end of object
			}
	}
	...
	private static String convertCode(String code) throws FileNotFoundException, ScriptException {
        ...
        System.out.println("Error "+numberOfErrors+":"+engine.get("error").toString() +" in "+currentObject+" event:"+eventname); 
        numberOfErrors++;
	...
	private static void populateSprites(BufferedWriter loadingfile) throws IOException {
		...
		for (int s = 0; s < size; s++)
			...
			if (s!=0) loadingfile.write(",");
			loadingfile.write(is.getId()+":{name:\""+is.getName()+"\",id:"+is.getId()+",transparent:"+is.get(PSprite.TRANSPARENT));

			loadingfile.write(",mask:"+is.get(PSprite.SEPARATE_MASK)+",smooth:"+is.get(PSprite.SMOOTH_EDGES));
			
			loadingfile.write(",originX:"+is.get(PSprite.ORIGIN_X)+",originY:"+is.get(PSprite.ORIGIN_Y));
			loadingfile.write(",bbLeft:"+is.get(PSprite.BB_LEFT)+",bbRight:"+is.get(PSprite.BB_RIGHT)+",bbTop:"+is.get(PSprite.BB_TOP)+",bbBottom:"+is.get(PSprite.BB_BOTTOM));
			loadingfile.write(",subImageCount:"+is.subImages.size());
			int subimages=is.subImages.size();
			if (subimages == 0) continue;
		
			/*
			 * Create an offscreen canvas
			 */
			BufferedImage offscreenimage = new BufferedImage((is.subImages.getWidth()*subimages), is.subImages.getHeight(), BufferedImage.TYPE_INT_ARGB);
			...
			for (int i = 0; i < is.subImages.size(); i++)
		        ...
		        spritesheet.drawImage(img, (is.subImages.getWidth()*i), 0, null);
